  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        } , icon: Icon(Icons.menu,size: screenWidth/10,color:Color(0xffffffff),)),
        title: Text('Home',
            style: Theme.of(context).textTheme.headlineMedium), centerTitle: true,
      ),
      body:
    ...
    if (currentUser == null) {
      return Center(child: Text('Please log in to interact with products.'));
    }
    ...
    return
        ...
        itemCount: products.length, // Number of items dynamically determined
        itemBuilder: (context, index) {
          final product = products[index];
         final data = product.data() as Map<String, dynamic>;

          final List<dynamic> imageList = data['imageUrl'] ?? [];
          final String firstImage = (imageList.isNotEmpty) ? imageList[0] : '';
                          ...
                          Row(
                            ...
                            children: [
                              LikesButton(
                                ...
                                likedBy: List<String>.from(data['likedBy'] ?? []), // قائمة المستخدمين الذين أعجبهم المنتج
                              ),
...
class _LikesButtonState extends State<LikesButton> with SingleTickerProviderStateMixin {
  late int _likes;
  late bool _isLiked;
  ...
  void initState() {
    ...
    _isLiked = widget.likedBy.contains(widget.userId); // التحقق إذا كان المستخدم قد تفاعل
